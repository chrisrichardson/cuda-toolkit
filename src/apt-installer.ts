...
export async function aptInstall(
  ...
  if (subPackages.length === 0) {
    ...
    return await exec(`DEBIAN_FRONTEND=noninteractive sudo apt-get -yq install`, [packageName])
  } else {
    ...
    return await exec(`DEBIAN_FRONTEND=noninteractive sudo apt-get -yq install`, versionedSubPackages)
  }
}
